...
Нет правильных или неправильных решений — мы просто соединяем детальки нашего конструктора, как нам нравится. Обычно лучше группировать функции так, чтобы их можно было переиспользовать, как `last` и `angry`. Если вы знакомы с книгой Мартина Фаулера «[Рефакторинг][refactoring-book]», вы можете узнать в этом прием «[извлечение функции][extract-function-refactor]» *(ранее — «извлечение метода»)*, за исключением того, что в нашем случае нет какого-либо состояния объекта, о котором пришлось бы беспокоиться.
