...
Нет правильных или неправильных решений — мы просто соединяем детальки нашего конструктора так, как нам понравится. Обычно лучше группировать функции так, чтобы их можно было переиспользовать, как `last` и `angry`. Если вы знакомы с книгой Мартина Фаулера «[Рефакторинг][refactoring-book]», вы можете узнать этот процесс как «[извлечение функции][extract-function-refactor]» *(ранее — «извлечение метода»)*, за исключением того, что в нашем случае нет какого-либо состояния объекта, о котором пришлось бы беспокоиться.
...
Заметили, как мы частично применили `replace`? Мы передаём данные от функции к функции, каждая из которых принимает только 1 аргумент. Каррирование позволяет нам подготовить каждую функцию так, чтобы она ожидала только данные, производила с ними действия, и передавала их дальше. Ещё стоит отметить, что нам не нужны данные для построения нашей функции в бесточечной версии, тогда как в обычной версии нам требуется объявить `word`, прежде чем сделать хоть что-то.
...
// бесточечный стиль
// Обратите внимание: вместо `join` мы используем функцию `intercalate`, определенную в приложении к книге (функция `join` предназначена для других задач и будет подробно рассмотрена в Главе 09)
const initials = compose(intercalate('. '), map(compose(toUpperCase, head)), split(' '));

initials("hunter stockton thompson"); // 'H. S. T'
```

Итого: бесточечный стиль может помочь нам убрать излишнее именование значений, что сохранит краткость и обобщённость. Это также индикатор функционального кода, поскольку он делает очевидным, что программы состоят только из небольших функций, которые принимают и возвращают данные. К примеру, не получится просто так использовать цикл `while` в композиции. Однако, имейте в виду, что бесточечный код — это обоюдоострый меч и иногда он может сделать намерение одного разработчика неочевидными для других. Не весь функциональный код обязан быть бесточечным, и это нормально. Мы будем придерживаться этого стиля по мере возможности, а в остальных случаях будем использовать обычные функции.

## Дебаггинг

Распространенной ошибкой является попытка использовать функцию двух аргументов (например, `map`) в композиции, забыв сперва частично применить её.

```js
// не верно — мы передаём массив в функцию angry, получаем непонятно что, а затем частично применяем к этому map
...
Композиция послужит нам хорошим инструментом, и, что особенно хорошо, она подкрепляется серьёзной теорией, которая послужит нам гарантией надежности. Давайте рассмотрим эту теорию.
...
Теория категорий достаточно абстрактна, чтобы смоделировать множесто вещей, но мы рассмотрим её в применении к тому, чем мы занимаемся: к типам и функциям.

**Коллекция объектов**

В роли объектов будут типы данных, такие как: ``String``, ``Boolean``, ``Number``, ``Object``. Мы часто рассматриваем тип данных как набор возможных значений — например, тип данных ``Boolean`` как множество значений `[true, false]` или ``Number`` как множество всех возможных числовых значений. Рассматривать типы как множества удобно, потому что позволяет применять к ним теорию множеств.
...
Это, как вы могли догадаться, и есть наша новая игрушка — `compose`. Не случайно мы обсуждали, что функция `compose` ассоциативна, так как это свойство обязано быть соблюдено для любой композиции в теории категорий.
...
Функция `id` обязана сочетаться с композицией. Вот свойство, которое в обязательно должно выполняться для каждой **унарной** функции `f` *(т.е. определённой для одного аргумента. **Арность** функции — это количество её аргументов — прим. пер.)*:
...
Напоминает свойство нейтрального элемента для обычных чисел, не так ли? Если это не сразу понятно, задержитесь на этом моменте. Оцените «безрезультатность» этой `id`. Скоро мы будем встречать `id` повсеместно, но пока что мы смотрим на `id` как на функцию, значение можно легко заменить аргументом. Это пригодится для написания бесточечного кода.

Это было кратким описанием теории категорий типов и функций. Если вы познакомились с ней впервые, то, полагаю, вам может быть ещё не до конца ясно что такое категория и чем она может быть полезна. Мы ещё не раз вернемся этой теме в этой книге. На данный момент, в этой главе, на этой строке, теория категорий по крайней мере даёт нам некоторую мудрость в отношении композиции, а именно, свойства ассоциативности и тождества.
