...
Мы разобрались, как писать программы, которые передают данные через ряд чистых функций. Программы, которые представляют из себя декларативное описание поведения. Но что насчет потока управления, обработки ошибок, асинхронных действий, состояния и, осмелюсь спросить, эффектов? В этой главе мы изучим основу для воплощения всех этих полезных абстракций.
...
* Будучи помещенными в «Контейнер», значение остается там насовсем. Мы *могли бы* получать доступ к ним, используя `.$value`, но такая практика только отдаляет нас от целей, ради которых создаются контейнеры.
...
Выглядит подобно функции `map`, определённой для массивов, только вместо `[a]` наша `map` определена для `Container a`. И работает так же:
...
Обратите внимание: наше приложение не упадет с ошибкой, когда мы вызывем фукнкцию `map` на нулевых значениях. Всё потому что `Maybe` будет каждый раз проверять, есть ли значение, перед тем как вызвать функцию.
