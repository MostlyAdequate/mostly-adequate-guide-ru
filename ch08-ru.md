...
Мы видели как писать программы которые передают данные через ряд чистых функций. Они декларативно описывают поведение. Но как на счет управления потоком, оброботки ошибок, асинхронности, состояния, и cмею сказать эфектов?! В этой главе, мы узнаем основу на которой построены все эти полезные абстракции.
...
Сначала мы создадим контейнер. Этот контейнер должен уметь содержать значения любых типов; контейнер который умеет содержать только один определенный тип редко бывает полезен. Контейнер это объект, но мы не будет добавлять ему никаких свойств или методов как в объектно ореинтированном подходе.Отнюдь, Мы будем думать о нем как о сундуке с сокровищами - специальная коробка которая содержит наши ценные данные.
...
Вот наш первый контейнер. Мы дали ему логичное название `Container`. Мы будем использовать `Container.of`  как конструктор который спасет нас от необходимости писать повсюду это отвратительное ключевого слова `new` . Функция `of`  больше чем кажется на первый взгляд, но на данный момент думайте об этом как о правильном способе помещения значения в наш контейнер.
...
Если вы используете node, то вы увидите `{$value: x}` несмотря на то, что на самом деле у нас `Container(x)`. Chrome будет выводить типы правильно, но не в этом суть; До того момента пока мы понимаем что из себя представляет `Container`, у нас все будет отлично. В некоторых средах выполнения JS мы могли бы просто написать свой вариант функции `inspect` если бы захотели, но мы не будем такими дотошными. В этой книге, мы будем указывать абстрактный вывод содержимого контейнера как если бы мы на самом деле переписали функцию `inspect` поскольку это гораздо более полезно чем `{$value: x}` как с педагогической, так и с эстетической точки зрения.

Let's make a few things clear before we move on:

Давайте проясним некоторые вещи перед тем как двигаться дальше:
...
* `Container` это объект с одним свойством. Во множестве контейнеров мы будем хранить всего одну вещь, хотя можем хранить и больше. Мы назвали это свойство `$value`.

* The `$value` cannot be one specific type or our `Container` would hardly live up to the name.

* `$value` значение не может быть определенного типа так как в таком случае название `Container` не соответсвует дейсвительности.

* Once data goes into the `Container` it stays there. We *could* get it out by using `.$value`, but that would defeat the purpose.

* Как только данные попадают в `Container` они там и остаются. Мы *могли бы* получить к ним доступ с помощью свойства `.$value`, но это нарушает саму цель контейнера.

The reasons we're doing this will become clear as a mason jar, but for now, bear with me.

Причины подобных свойств контейнера скоро станут ясны как белый день, но пока что я прошу вас немного потерпеть.
...
Как только наше значение каким бы оно не было попало в контейнер, на нужен способ как мы можем применить функции к этому значению.
...
Выглядит совсем как давно известная функция `map` в массивах, за исключением того что у нас `Container a` вместо `[a]`. Но по существу работает так же:
...
Мы можем работать с нашим значением без необходимости покидать наш `Container`. Это замечательно. Наше значение внутри `Container` передается в `map` фунцию таким образом мы можем обработать его и после этого возвращать в `Container`  на хранение. В результате того, что значение всегда остаётся внутри `Container` мы можем продолжать использовать `map` с любыми функциями по желанию. Мы можем даже изменить тип входного значения как показно в третьем примере выше.
...
Минуточку, если мы продолжим вызывать функцию `map`, то это похоже на своего рода композицию! Какое математическое волшебство происходит здесь? Итак народ, Мы только что открыли *Functors*.
...
Да, *Functor* это просто интерфейс с каким-то контрактом. Мы могли бы так же просто назвать его *Mappable*, но это было бы слишком скучно. Функторы пришли из теории категорий и мы посмотрим на них в деталях с математической точки зрения в конце этой главы, сейчас же давайте воспользуемся интуицией и посмотрим как можно применять этот интерфейс с странным названием.
...
Какие причины могут быть для того что бы оборачивать значение и использовать `map` для того что бы работать с ним? Ответ появится если мы зададим правильный вопрос: В чем приемущество если мы будем просить наш контейнер применить функции за нас? Дополнительный уровень абстракции при применении функции. Когда мы передаем функцию в `map` мы просим контейнер вызвать её для нас. На самом деле это очень мощная концепция.
...
`Container` довольно скучный. На самом деле, обычно он называется `Identity` и имеет тот же эффект, что и функция `id`.( снова таки сущетсвует связь с математикой и мы рассмотрим это когда придет время). Однако, существуют други функторы, то есть другие типы контейнеров, которые имеют собственную реализацию функции `map`, которая дает очень полезное поведение при вызове `map`. Давайте определим один прямо сейчас.

> Польная реализация приведена в  [Appendix B](./appendix_b.md#Maybe)
...
Теперь, `Maybe` очень похож на `Container` с одним незначительным изменением: Он сначала проверит есть ли значение перед тем как вызвать переданую функцию. Таким образом, мы можем избежать раздражающих null и undefined при использовании `map` (Обратите внимание что реализация упрощена для учебных целей).
...
Обратите внимание наше приложение не упало с ошибкой, когда мы вызывем фукнкцию `map` на нулевых значениях. Всё потому что `Maybe` будет каждый раз проверять есть ли значение перед тем как вызвать функцию.
...
Вызов функций через `.` работает просто прекрасно, но по причинам упомянутым в первой части книги, мы бы хотели писать код в стиле отсутствия ссылок на данные. На самом деле, `map` полностью готов к тому, чтобы делегировать вызов функции любому функтору, который мы ему передадим:
...
Этот восхитительно поскольку мы можешь продолжать использовать композицию и `map` будет работать как мы ожидали. Так же работает и функция `map` из библиотеки ramda. Мы будем использовать вызов функций через `.` в учебных целях когда он боллее информативен а для удобства будем пользоваться стилем с отсутствием ссылок на данные. Вы заметили? Я незаметно ввел дополнительную новую нотацию для описания сигнатуры функции: `Functor f =>` говорит нам что `f` должна быть Функтором. Вроде бы не сложно, но я понимал, что небольшое объяснение не повредит.
...
На практике, мы будем встречать `Maybe` в фунцкиях которые могут и не вернуть результат.
...
`safeHead` похож на нашу функцию `head`, но с добавление гарантии безопасности типа. Происходит интерсная вещь когда мы добавляем `Maybe` в наш код; Мы вынуждены иметь дело с `null` значениями.  `safeHead` функция честна с нами и предупреждает заранее, что может не вернуть значение, в этом нет ничего такого, по этой причние она возвращает `Maybe` что бы проинформировать нас об этом. Более того, она не только информирует нас о потенциально неправильном возвращаемом значение, но также требует от нас использования `map` для доступа к возвращаемому значению, поскольку он спрятан внутри `Maybe`. Вообще говоря, эта проверка на `null` существует внутри самой функции `safeHead`. Мы можем теперь спать спокойно ночью значая что  `null` не появится когда мы меньше всего этого ожидаем. Подобный методы позволяют нам совершить переход нашего приложения от сделанного из бумаги и заклёпок к дереву и гвоздям. Подобный подход гарантирует более надёжный код.
...
`withdraw` начнет крутить носом и вернет `Nothing` если у нас мало денег. Эта функция также сообщает о ее непостоянстве и не оставляет нам выбора, кроме как `map`'ить все что она возвращает. Разница в том что `null` был использован намеренно здесь. Вместо `Just('..')`, мы получаем `Nothing` как сигнал об ошибки и эффективно останавливаем наше приложение. Важно отметить: Если `withdraw` не будет выполнен успешно, тогда `map`  прервёт оставшуюся часть наших вычислений т.к. переданная функция не будет запущена, а именно `finishTransaction`. Это именно то поведение которое и задумывалось, поскольку мы предпочли бы не обновлять наш бух.книгу учета или отображать новый баланс, если вычет средств не был успешным.
...
Одна вещь, которую люди часто упускают, — это то, что когда-нибудь наступит конец; какая-нибудь функция с эффектами, например: отправляющая JSON, или выводящая данные на экран, или меняющая файловую систему или что-то еще. Мы не можем отдавать результат с помощью `return`, мы должны запустить одну или несколько функций для того, чтобы передать наш результат миру. Можно сформулировать это как Дзен-буддистский коан: : "Если у программы нет наблюдаемого эффекта, работает ли она вообще?". Или вы достигли своей цели после запуска? Возможно, она просто потратила несколько процессорных циклов и заснула...
...
Тем не менее есть одна лазейка. Если вы хотите вернуть кастомное значение и продолжить, вы можете сделать это с помощью небольшой функцией которая называется `maybe`.
...
Теперь мы либо вернём статическое значение (того же типа, что и возвращает finishTransaction), либо продолжим, завершение транзакции без `Maybe`. С `maybe`, мы наблюдаем эквивалент оператора  `if/else`, тогда как с `map`, императивным аналогом было бы: `if (x !== null) { return f(x) }`.
...
Знакомство с `Maybe` может вызвать некоторый дискомфорт по началу. Пользователи Swift и Scala знают что я имею ввиду т.к это зашито во встроеные библиотеки этих языков в виде `Option(al)`. Когда приходится всё время иметь дело с проверкой на null (даже когда мы знаем, что значение просто не может быть равным нулю) многие люди считают это очень трудоемким.Как бы то ни было, со временем, это станет нормой для вас и вы, вероятно, оцените такую страховку. В конце концов, большую часть времени это будет очень полезно и спасёт наши жизни.
...
Написание небезопасных программ, это тоже самое что старательно разукрашивать яйца перед тем как бросить их на дорогу; это как строить дом престарелых из материалов которыми строили три поросёнка. Будет хорошо, если мы добавим некоторую безопасность в наши функции и `Maybe` именно в этом нам и помогает.
...
Было бы упущением с моей стороны не упомянуть, что "настоящая" реализация делит `Maybe` на два типа: один для любых данных, а второй для их отсутствия. Это позволяет нам подчиняться свойству параметричности в `map`, и благодаря этому, такие значения как `null` и `undefined` так же могут быть обработаны и универсальная квалификация значения в функторе будет соблюдена. Вы часто будете видеть такие типы как `Some(x) / None` или `Just(x) / Nothing` вместо `Maybe`, который делают проверку на `null` своего значения.
...
Это может стать шоком, но `throw/catch` не совсем то и чистый. Когда выбрасывается исключение, вместо того, чтобы вернуть значение, мы бьём тревогу! Функция атакует, извергая тысячи нулей и единиц, как щиты и копья что похоже на электронную войну против вторжения входных значений. C нашим новым другом `Either`, мы можем сделать лучше чем просто объявить войну входным данным, мы можем ответить вежливым сообщением. Давайте посмотрим на пример:
...
`Left` и `Right` это два подкласса абстрактного класса который называется `Either`. Я пропустил церемонию создания суперкласса `Either` так как мы не будем его использовать, но хорошо знать что он есть. Итак, здесь нет ничего нового, кроме двух типов. Давайте посмотрим как они работают:
...
`Left` как подросток, игнорирует наши просьбы обработать значение через `map`. `Right` будет работать точно так же как `Container` (он же Identity). Мощь заключается в том у нас есть возможность встроить сообщение об ошибки внутр `Left`.
...
Предположим у нас есть функция которая может завершиться с ошибкой. Например, вычисление возраста по дате рождения. Мы могли бы использовать `Nothing`/`Maybe(null)` что бы сообщить об  ошибке и перенаправить нашу программу, однако, это мало о чем нам скажет. Возможно, мы бы хотели  знать почему произошла ошибка. Давайте реализуем это используя `Either`.
...
Теперь, Прямо как `Nothing`/`Maybe(null)`, Мы упрощаем приложения возвращая `Left`. Разница в том что, теперь мы имеем понимание о том, почему наша программа сорвалась. Стоит заметить что мы возвращаем `Either(String, Number)`, который содержит `String` как его левое значени и  `Number` как правое `Right`. Эта сигнатуры функции немного неформальная поскольку мы не нашли времени что бы определить родителский класс `Either`, одако, мы можем многое подчерпнуть из этого типа. Он говорит нам, что мы получаем сообщение об ошибке или возраст.
...
Если `birthDate` валидное значние, то программа выведет на экран таинтсвенное значение(ваш возраст) что бы мы смогли его созерцать. Иначе, он покажет вам `Left` c сообщение об ошибке, ясное как божий день, хотя и по прежнему внутри вашего контейнера. Таким образом, это показывает нам ошибку, но в спокойной и вежливой форме, в отличие от ребенка, который теряет контроль, когда что-то идет не так.
...
В этом примере, мы логически  выбираем нашу ветку выполнения в завимости от того валидные ли данные о дне рождения или нет, мы движемся линейно справа налево одним движением, вместо того что бы поднимаемся через условные операторы.  Как правило, мы обычно выносим  `console.log` из таких функции как `zoltar` и обрабатываем её с помощью `map` в момент вызова, но в даном случае это полезно видеть как  `Right` ветка отличается. Мы используем `_` в правой части сигнатуры функции для того что бы указать что это значение которое должно быть проигнорировано (В некоторых браузерах вам нужно будет использовать `console.log.bind(console)` что бы вы смогли использовать его как first class object).
...
Я бы хотел остановится на секунду, чтобы подчеркнуть то, что вы, возможно, пропустили: `fortune`, несмотря на его совместное использование с `Either` в этом примере, не знает ничего о какой-либо связи с функтором. Тоже самое было и в случае с `finishTransaction` в предыдущем пример. Во время вызова, функция может быть обернута в `map`, который траснформирует её  из той которой не связана с функтором в ту которая в функторе. Мы называем этот процесс *lifting/подъем*. Часто функции проще работать с обычными типами, а не с контейнерами, а затем подниматься в нужный контейнер, если это необходимо. Это придает простоту и делает функции более многоразовыми, теми которые могут быть изменены для работы с любым функтором по требованию.
...
`Either` отлично подходит как для обычных ошибок, таких как валидационные проверки, так и для более серьезных вещей, напрмире недостающие файлы или проблем с сокетами. Попытайтесь заменить в некоторых из примеров `Maybe` на `Either` что бы получить лучший отклик.
...
На данный момент, Я немогу не думать о том что я сделал `Either` медвежью услугу когда представил его как просто контейнер для сообщений об ошибаках. `Either` представляет логическое ИЛИ (то есть `||`). Он также воплощает идею `Coproduct` из теории категорий (мы не будем рассматривать её в этой книге, но её стоит прочитать, потому что она имеет много свойств для изучения). Кроме того, это стандартный тип суммы (или несвязанный союз множеств), поскольку общее количество всех возможных значений, которые оно содержит, является суммой двух его типов (я знаю, что вы читате это но не понимаете, вот очень хорошая статья об этой проблеме [great article](https://www.fpcomplete.com/school/to-infinity-and-beyond/pick-of-the-week/sum-types)).  Есть много вещей которые `Either` может сделать но как функтор, он используется для обработки ошибок.
...
Как и с `Maybe`, у нас есть мини `either`, который ведет себя похожим образом, он принимает 2 фунции вместо одной и еще статическое значение. Каждая функция должна возвращать один и тот же тип:
...
Наконец использование этой таинственной функции  `id` .На самом деле, она просто копирует значение из `Left`,в сообщение об ошибке а затем это значение передается в `console.log`. Мы сделали наше приложение для гадания более надежным, обеспечив обработку ошибок внтури `getAge`. Мы или ставим пользователя перед фактом, раскрывая ему чистую правду о его ошибке или выполняем последовательность ожидаемых действий. Теперь мы готовы перейти на новому семейству функторов.
...
Внутренний механизм этого подхода немного сложно разместить на этой странице, поэтому мы будем использовать Data.Task(ранее Data.Future) знакомтесь Quildreen Motta и его фантастическая библиотеку [Folktale](http://folktale.origamitower.com/).
...
// Вы можете передавать и обычные значения
...
Используйте `add` и `map` для того что бы написать фунцию которая увеличивает значени на единцу внутри функтора.
...
Используя следующий обект пользователя:
...
Напишите функцию которая использует `checkActive` и `showWelcome` что бы предоставить доступ или вернуть ошибку.
...
Теперь рассмотрим следующию функцию:
...
Напишите функцию `validateName` которая проверяют длину имени пользователя и если длина меньше чем 3 символов то возвращает ошибку. Затем используейте `either`,  `showWelcome` и `save` что бы написать функцию `register` что бы зарегистрировать и поприветствовать пользователя если он прошел валидацию. Помните, что оба аргумента `either` должны возвращать один и тот же тип.
