...
На практике обычно мы будем встречать `Maybe` в таких функциях, для которых вычисление результата возможно не всегда.
