...
Существует доступная оптимизация – мы проходимся функцией `map` по каждому элементу, чтобы взять из его свойств адрес изображения, а затем мы проходимся по массиву адресов, чтобы сформировать из них <img>. Есть закон, описывающий композицию отображений:
...
Мы можем использовать это свойство для оптимизации кода. Давайте начнём принципиональный рефакторинг.
...
Использовав наши новые знания мы построили маленькое, но практичное приложение. Основываясь на математических законах мы отрефакторили код. Но что насчёт обработки ошибок и ветвления кода? Как мы можем чделать целое приложение чистым, вместо отделения нечистых функций в отдельное пространство имён? Как нам сделать наше приложение более выразительным и надежным? Ответы на эти вопросы мы рассмотрим во второй части книги.
