...
Вспомните SQL. Там нет "Сначала сделай то, затем сделай это". Есть выражение, которое определяет, что бы мы хотели сделать с базой данных. Мы не решаем как сделать работу, выражение делает это. Когда база данных обновилась и движок SQL оптимизировался, нам не приходится редактировать запросы. Всё происходит так, потому что существует много путей интерпретировать наши условия и получить такой же результат. 

Поначалу, для многих, включая меня, было сложно понять концепцию декларативного программирования, поэтому давайте обратим внимание на несколько примеров, чтобы прочувствовать его. 
...
Императивный цикл сначала инициализирует массив. Интерпретатор должен выполнить это выражение перед тем как двигаться дальше. Затем он непосредственно проходит по списку автоомобилей, вручную увеличивая счетчик, открыто показывая нам каждую часть итерации.
...
В дополнение, более ясно и кратко — функция `map` может быть по желанию оптимизирована, но код нашего драгоценного приложения не будет нуждаться в исправлениях.
...
## Функционально запрограммированный flickr 

Сейчас мы построим пример приложения декларативным, компонуемым способом. Пока что мы схитрим и будем использовать побочные эффекты, но мы будем использовать их по-минимуму и отделять от нашей базы чистого кода. Мы собираемся написать виджет для браузера, который парсит изображения из flckr и выводит их на экран. Начнем с каркаса приложения. Это html разметка: 
...
А это основа flickr.js: And here's the flickr.js skeleton:
