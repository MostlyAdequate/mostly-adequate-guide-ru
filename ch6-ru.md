...
Все, что мы сделали, это очередная композиция функций, которая вызовет `mediaUrls` и наполнит ими содержимое `<main>`. Мы заменили вызов `trace` на` render`, так что теперь нам есть что показать помимо содержимого json — сейчас это просто ссылки на изображения.
