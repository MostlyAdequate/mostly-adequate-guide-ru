...
Наконец, в случае `then` `Promise p =>` говорит нам, что `p` должно быть Promise, что означает, что типы `p a` и` p b` будут Promise, предоставляющими значения типов `a` и` b` соответственно. То же самое касается других стандартных встроенных объектов, таких как массив.
